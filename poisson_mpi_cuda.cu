...
static Domain decompose2d(int M, int N, int size, int rank) {
  ...
  if (d.rankx < extrax) {
      d.istart = d.rankx*(basemx+1) + 1;
      d.iend = d.istart + (basemx+1) - 1;
  } else {
      d.istart = extrax*(basemx+1) + (d.rankx-extrax)*basemx + 1;
      d.iend = d.istart + basemx - 1;
  }

  if (d.ranky < extray) {
      d.jstart = d.ranky*(basemy+1) + 1;
      d.jend = d.jstart + (basemy+1) - 1;
  } else {
      d.jstart = extray*(basemy+1) + (d.ranky-extray)*basemy + 1;
      d.jend = d.jstart + basemy - 1;
  }
...
__global__ void kernel_compute_coeff_F(
  ...
  int M, int N,
  double h1, double h2, double eps,
  double* a, double* b, double* F)
{
  ...
  double fval = 0.0;
  if (gi >= 1 && gi <= M-1 && gj >= 1 && gj <= N-1)
      fval = inDomain(x, y) ? 1.0 : 0.0;
  F[idx] = fval;
}
    ...
    const double* w, double* out)
{
  ...
  double val = -(a[ip] * (w[ip] - w[c]) - a[c] * (w[c] - w[im])) / h1sq
    ...
    Timers* timers)
{
  ...
  double t0 = MPI_Wtime();
  CUDA_CHECK(cudaMemcpy(hPartials, dPartials, nThreadsTotal*sizeof(double),
...
struct HaloBuffers {
  // device
  double *d_sendL=nullptr, *d_sendR=nullptr, *d_recvL=nullptr, *d_recvR=nullptr;
  double *d_sendB=nullptr, *d_sendT=nullptr, *d_recvB=nullptr, *d_recvT=nullptr;
  // host pinned
  double *h_sendL=nullptr, *h_sendR=nullptr, *h_recvL=nullptr, *h_recvR=nullptr;
  double *h_sendB=nullptr, *h_sendT=nullptr, *h_recvB=nullptr, *h_recvT=nullptr;
};
    ...
    Timers* timers)
{
  ...
  cudaEventRecord(evStart);
  kernel_pack_LR<<<(d.localN + tpb - 1)/tpb, tpb>>>(d.localM, d.localN, dw,
                                                      hb.d_sendL, hb.d_sendR);
  kernel_pack_BT<<<(d.localM + tpb - 1)/tpb, tpb>>>(d.localM, d.localN, dw,
                                                      hb.d_sendB, hb.d_sendT);
  cudaEventRecord(evStop);
  CUDA_CHECK(cudaEventSynchronize(evStop));
  float msPack = 0.0f;
  ...
  MPI_Sendrecv(hb.h_sendL, d.localN, MPI_DOUBLE, d.left, 10,
                hb.h_recvL, d.localN, MPI_DOUBLE, d.left, 11, comm, &st);
  MPI_Sendrecv(hb.h_sendR, d.localN, MPI_DOUBLE, d.right, 11,
                hb.h_recvR, d.localN, MPI_DOUBLE, d.right, 10, comm, &st);
  MPI_Sendrecv(hb.h_sendB, d.localM, MPI_DOUBLE, d.bottom, 20,
                hb.h_recvB, d.localM, MPI_DOUBLE, d.bottom, 21, comm, &st);
  MPI_Sendrecv(hb.h_sendT, d.localM, MPI_DOUBLE, d.top, 21,
                hb.h_recvT, d.localM, MPI_DOUBLE, d.top, 20, comm, &st);
  double t3 = MPI_Wtime();
  ...
  cudaEventRecord(evStart);
  kernel_unpack_LR<<<(d.localN + tpb - 1)/tpb, tpb>>>(d.localM, d.localN, dw,
                                                        hb.d_recvL, hb.d_recvR,
                                                        hasLeft, hasRight);
  kernel_unpack_BT<<<(d.localM + tpb - 1)/tpb, tpb>>>(d.localM, d.localN, dw,
                                                        hb.d_recvB, hb.d_recvT,
                                                        hasBottom, hasTop);
  cudaEventRecord(evStop);
  CUDA_CHECK(cudaEventSynchronize(evStop));
  float msUnpack = 0.0f;
...
int main(int argc, char** argv) {
  ...
  dim3 blk2(16,16);
  dim3 grdCoeff((d.localN+2 + blk2.x-1)/blk2.x,
                (d.localM+2 + blk2.y-1)/blk2.y);

  cudaEventRecord(evStart);
  kernel_compute_coeff_F<<<grdCoeff, blk2>>>(
      d.localM, d.localN, d.istart, d.jstart, M, N, g.h1, g.h2, g.eps,
      da, db, dF);
  cudaEventRecord(evStop);
  CUDA_CHECK(cudaEventSynchronize(evStop));
  float ms_coeff = 0.0f;
  ...
  // ---- Solve: CG with first iteration OUTSIDE the loop ----
  MPI_Barrier(MPI_COMM_WORLD);
  double t_total_start = MPI_Wtime();

  dim3 grdOp((d.localN + blk2.x-1)/blk2.x,
              (d.localM + blk2.y-1)/blk2.y);
  ...
  CUDA_CHECK(cudaEventSynchronize(evStop));
  float msA = 0.0f;
  CUDA_CHECK(cudaEventElapsedTime(&msA, evStart, evStop));
  ...
  CUDA_CHECK(cudaEventSynchronize(evStop));
  float msRinit = 0.0f;
  CUDA_CHECK(cudaEventElapsedTime(&msRinit, evStart, evStop));
  ...
  CUDA_CHECK(cudaEventSynchronize(evStop));
  float msD = 0.0f;
  CUDA_CHECK(cudaEventElapsedTime(&msD, evStart, evStop));
